<?php namespace Sereno\Extensions\Projects;
...
class ProjectsBuilder implements Builder
{
    ...
    public function __construct(DataExtractor $extractor, ProcessorFactory $factory, FrontParser $markdown)
    {
        ...
        $this->projectDirectory = config('projects.directory');
        $this->baseUrl = config('projects.url_prefix');
    }
    ...
    public function build(array $files, array $data)
    {
        ...
        $options = [
            'view' => [
                'extends' => config('projects.extends'),
                'yields'  => config('projects.yields'),
            ],
            'interceptor' => [$this, 'getOutputFilename'],
        ];

        $file = $this->getView('projects.index');
